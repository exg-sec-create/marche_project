...
## イベント参加ボタン
- QRチケット下部のイベント参加ボタンは受付完了後にだけ有効になります。
- イベント参加時に来場者が画面をスタッフへ提示し、スタッフが確認ダイアログを読んでボタンを押します。
- 参加記録は Firestore のトランザクションとセキュリティルールで一度だけ書き込まれ、参加後はボタンが無効になります。
- 既存の登録データも未参加として扱うため、すでに発行済みのQRチケットにも自動で表示されます。
- この機能を本番で利用する前に、下記コマンドで最新の `firestore.rules` を必ず反映してください。
